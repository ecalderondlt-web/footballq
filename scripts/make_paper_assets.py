...
def gather() -> dict[str, Any]:
    data = {
        ...
        "panel_enrichment": {},
        "variant_count": len(
            list((ROOT / "configs").glob("td_jepa_nonoverlap*_skillcorner.yaml"))
        ),
    }
...
def write_numbers(data: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        macro("NumMatches", 10),
        macro("NumTrainMatches", 6),
        macro("NumValMatches", 2),
        macro("NumTestMatches", 2),
...
def write_gate_table(data: dict[str, Any]) -> None:
    ...
    for gate_name, payload in (gate.get("gates", {}) or {}).items():
        ...
        blocker_text = (
            r"\texttt{" + r"}, \texttt{".join(tex_escape(b) for b in blockers) + r"}"
            if blockers
            else "none"
        )
        lines.append(
...
def write_cleanroom_table(data: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"    probe blocker count & {len(ORIGINAL_RUN['probe_blockers'])} &"
        f" {len(set(b.split(':', 1)[1] if ':' in b else b for b in probe_blockers))} targets"
        f" ({len(probe_blockers)} conditions) & see text \\\\",
        ...
        r"    \bottomrule",
        r"  \end{tabular}",
        r"  \caption{Clean-room reproduction: gate outcomes from the original run"
        r" (reported in the repository's integrity documentation at commit"
        r" \texttt{acb2ba3}) versus this paper's independent re-execution on different"
        r" hardware and library versions. We compare gate outcomes and named blockers,"
        r" not floating-point losses.}",
